...
EOD
    ...
    context 'for expected values of properties' do
      properties.keys.each do |key|
        it "should be able to set the #{key} property" do
          expect(provider.class).to receive(:get_configuration).and_return(
  ...
  describe 'self.prefetch' do
    ...
    it 'can be called on the provider' do
      expect(provider.class).to receive(:get_configuration).and_return(properties)
  ...
  describe 'get_configuration' do
    ...
    it 'parses the rhsm_repo_ca_cert property' do
      expect(File).to receive(:open).with('/etc/rhsm/rhsm.conf').and_return(StringIO.new(raw_data))
      expect(provider.class).to receive(:subscription_manager).with(['config', '--list']).and_return(raw_data)
      configs = provider.class.instances
      config = configs[0]
    ...
    it 'detects default options' do
      ...
      expect(resource.provider.class).to receive(:subscription_manager).with(['config', '--list']).and_return(raw_hostname01_data)
